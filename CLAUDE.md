...
- `LIVE_INTEGRATIONS_ENABLED` (`src/constants/liveIntegrations.js`) is the single switch for "do integrations with real external side effects actually fire, or fall back to a safe dev-mode stand-in?" — defaults to `import.meta.env.PROD`, overridable via `VITE_LIVE_INTEGRATIONS_ENABLED` for local testing. Currently gates `uploadToCloudinary`/`deleteFromCloudinary` (`src/utils/cloudinaryUpload.js` — returns a mock `picsum.photos` link and skips the real upload/delete outside prod) and `triggerExchangeRatesRefresh` (`src/composables/useExchangeRatesRefresh.js` — simply doesn't fire outside prod). Any new integration with a real external side effect should gate on this same flag rather than adding its own env check. `kharcafy-node-be` has the equivalent backend-side switch, `env.liveIntegrationsEnabled`, gating actual email sending — the two are independent (one per deployable), not a shared value.
