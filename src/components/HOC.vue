<template>
	<Suspense>
		<template #default>
			<component :is="componentToBeRendered" v-bind="componentProps" v-on="listenersToPass" ref="componentRef" />
		</template>
		<template #fallback>
			<div class="loading-wrapper" v-loading="true" element-loading-text-color="red" element-loading-text="Loading..." :element-loading-spinner="svg" element-loading-svg-view-box="-10, -10, 50, 50" element-loading-background="rgba(0, 0, 0, 0)"></div>
		</template>
	</Suspense>
</template>
